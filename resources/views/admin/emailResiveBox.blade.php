...
<form action='/emailDell' method='post'>
	{{csrf_field()}}
	<div class="cl pd-5 bg-1 bk-gray mt-20"> <span class="l"><input type='submit' value='批量删除' class="btn btn-danger radius" />  <a href="/emailAdd"  class="btn btn-primary radius"><i class="Hui-iconfont">&#xe600;</i> 发送邮件</a><a href="/emailOut"  class="btn btn-success radius"><i class="Hui-iconfont">&#xe600;</i> 导出邮件</a></span></div>
	<div class="mt-20">
	<table class="table table-border table-bordered table-hover table-bg table-sort">
		...
		<tbody>
			...
			<tr class="text-c">
				...
				<td>{{$k+1}}</td>
				<td>{{userInfo($v->from_id)->truename}}--{{userInfo($v->from_id)->username}}</td>
				<td>{{mb_substr($v->title,0,8)}}...</td>
				<td>{{$v->file_num}}</td>
				<td class="td-status"><a  href='/emailRead1/{{$v->id}}' >@if($v->is_read == 1)<span class="label label-success radius">已读</span>@elseif($v->is_read == 0) <span class="label label-danger radius">未读</span> @endif</a></td>
				<td>{{mb_substr($v->content,0,15)}}...</td>
				<td>{{date('Y-m-d H:i:s',$v->addtime)}}</td>
				<td class="td-manage"><a title="查看" href="/emailRead1/{{$v->id}}"  class="ml-5 label label-primary radius" style="text-decoration:none">查看</a><a title="删除" href="/emailDel/{{$v->id}}" class="ml-5 label label-danger radius" style="text-decoration:none"><i class="Hui-iconfont">删除</i></a>
